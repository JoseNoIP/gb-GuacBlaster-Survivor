...
30. **El archivo de traducciones NO puede tener extensión `.csv` en Android** — Bug conocido de Godot ([#38957](https://github.com/godotengine/godot/issues/38957)): los `.csv` son excluidos del export PCK incluso con `include_filter="*.csv"` porque Godot los trata como recursos de traducción procesados. **Solución**: usar extensión `.txt` (u otra no reconocida por Godot) y añadir `include_filter="*.txt"` en `export_presets.cfg`. El contenido del archivo puede seguir siendo CSV válido — solo cambia la extensión.
